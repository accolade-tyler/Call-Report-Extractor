...
# ==============================
# YOUR ACCOUNT CODE LIST (as provided)
# ==============================
...
# ==============================
# PDF → HTML (your logic, returning HTML string)
# ==============================
...
# ==============================
# HTML → rows (your extractor logic)
# ==============================
def extract_rows_from_html(html: str) -> List[Tuple[str, object]]:
    ...
    """
    soup = BeautifulSoup(html, 'html.parser')  # match your original parser choice
    tables = soup.find_all('table')
    if not tables:
        return []

    codes_set = set(account_codes)  # for O(1) membership checks
...
# ==============================
# BUILD EXCEL (sheets per PDF + Consolidated + Log)
# ==============================
...
with st.sidebar:
    ...
    include_consolidated = st.checkbox("Include Consolidated sheet", value=True)
    ...
    st.markdown("---")
    st.caption("Notes: This extractor matches cells equal to a known account code and captures the **previous column** value, mirroring your script.")
    ...
    for idx, (name, data) in enumerate(pdf_files, start=1):
        ...
        try:
            ...
            if rows:
                ...
                # Still add an empty sheet? Match your original: it skipped if empty.
                # We'll log and skip sheet creation to be faithful.
                pass
